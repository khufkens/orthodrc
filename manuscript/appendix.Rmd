...
ct <- crosstab(m, loss)
ct <- round(ct/sum(ct),2) * 100
rownames(ct) <- c("non-forest","forest")

ct %>%
   kable(
    caption="Contingency table in agreement (%) between forest / non-forest classes as generated from a Geo-Eye 1 pan-chromatic image using a Convolutional Neural Network (CNN) generated and the Global Forest Cover map (Hansen et al. 2013).",
    booktabs = TRUE,
